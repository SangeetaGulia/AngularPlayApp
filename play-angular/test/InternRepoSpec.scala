...
class InternRepoSpec extends Specification{
  ...
  "Intern repository" should {
    ...
    "Add intern test" in new WithApplication {
      val result = Await.result(internRepo.addIntern(Intern("aksh","aks@gmail.com",3256,"dabhri",354536)), 5 seconds)
      assert(result === 1)
    }

    "delete intern test" in new WithApplication {
      val result = Await.result(internRepo.deleteById(1),5 seconds)
      assert(result === 1)
    }
  }
}